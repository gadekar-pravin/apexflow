...
- **Firebase Auth:** Identity Platform enabled with Google sign-in provider. `authDomain` is dynamically resolved at runtime by `resolveAuthDomain()` in `AuthContext.tsx` — if the current hostname is in the `FIREBASE_HOSTING_DOMAINS` allowlist (`askcortex.dev`, `apexflow-console.web.app`), it uses that hostname so `signInWithRedirect` stays on the same origin; otherwise falls back to `VITE_FIREBASE_AUTH_DOMAIN` (defaults to `askcortex.dev`). This ensures sign-in works even when one domain is blocked (e.g. by a VPN). Using `firebaseapp.com` as authDomain breaks due to third-party cookie blocking. OAuth redirect URIs: `https://askcortex.dev/__/auth/handler`, `https://apexflow-console.web.app/__/auth/handler`. Authorized domains: `localhost`, `askcortex.dev`, `apexflow-console.web.app`, `apexflow-ai.firebaseapp.com`, `apexflow-ai.web.app`
...
**Firebase Authentication:** `AuthContext` (`contexts/AuthContext.tsx`) wraps the app with Firebase Auth state. Uses `signInWithRedirect` with Google provider (avoids cross-origin popup COOP issues — Google's sign-in page sets `COOP: same-origin` which blocks `window.closed` polling from `signInWithPopup`). `authDomain` is resolved dynamically via `resolveAuthDomain()`: if `window.location.hostname` is in the `FIREBASE_HOSTING_DOMAINS` allowlist, that hostname is used as `authDomain` so the redirect stays on the same origin; otherwise falls back to `VITE_FIREBASE_AUTH_DOMAIN`. `getRedirectResult()` handles the redirect return and catches errors. Token provider pattern decouples auth from the API layer — `setAuthTokenProvider()` in `services/api.ts` is called during AuthContext init, and `fetchAPI()` attaches tokens automatically. Every data-fetching component uses the guard pattern `!auth.isConfigured || auth.isAuthenticated` to enable auth-optional in dev and required in production. `AppShell.tsx` uses `GET /api/auth/verify` (no DB access) for the authorization check, so users can login and access DB-independent features (Settings, theme, SSE) even when AlloyDB is down. Shows a loading spinner during Firebase init (~200-500ms) to prevent flash of sign-in content. Cache invalidation via `queryClient.clear()` on user switch (skips first fire to avoid unnecessary clearing on page load). SSE connection (`SSEContext.tsx`) gates on auth state and passes token via `?token=` query param since EventSource can't send headers.

**Graceful degradation:** When the database is unavailable, the app still loads and navigates. `useDbHealth` hook (`hooks/useDbHealth.ts`) polls `/readiness` every 60 seconds (via `useSmartInterval`) and drives a DB indicator in `StatusBar.tsx` (green/red dot). Data-dependent pages (Dashboard, Documents) show their existing error states and auto-recover when the DB returns.

**Smart health polling:** Both `useApiHealth` and `useDbHealth` use the shared `useSmartInterval` hook (`hooks/useSmartInterval.ts`) to reduce Cloud Run costs. The hook pauses polling when the tab is hidden (Page Visibility API), stops polling after 5 minutes of user inactivity (mousemove/keydown/touchstart/scroll), and fires an immediate check on resume. Activity events are throttled to 1/second. Interval is 60 seconds (up from 30s). On mount, the hook checks `document.hidden` and starts paused with zero requests if the tab is already in the background (e.g. opened via middle-click or restored by the browser). Polling only begins when the tab becomes visible. A user who is active for 20 min then leaves the tab open for 8 hours goes from ~2,000 requests to ~40 (~98% reduction for background tabs).
...
**Chat page (`/` and `/chat`):** Conversational agent interface at `pages/ChatPage.tsx`. Mounted at both `/` (home) and `/chat` (deep-linkable). Components in `components/chat/`: `WelcomeScreen` (suggestion cards + inline input), `ChatMessageList` (markdown rendering via `react-markdown` with `extractDisplayContent()` to parse `markdown_report` from JSON agent output at render time), `ChatInput` (InsightAgent-style gradient glow, no focus ring), `RightPanel` (tabbed container with Activity and Charts tabs), `ReasoningSidebar` (live SSE timeline of agent execution, events consolidated by step — rendered inside RightPanel's Activity tab), `ChartCard` (single recharts chart renderer with validation and "Chart unavailable" fallback), `VisualizationCanvas` (collects all `metadata.visualizations` from messages, lazy-loaded via `React.lazy` to code-split recharts ~388KB), `ChatSessionList` (session CRUD). All three panels (sessions sidebar, main chat, right panel) use `ResizablePanel` with localStorage persistence (`apexflow.chat.sessionsWidth`, `apexflow.chat.reasoningWidth`).

**Chat orchestration:** User message → `chatService.addMessage()` (with local fallback on failure) → `runsService.create()` → independent poll loop via `activePollsRef` Set → extract `markdown_report` + `visualizations` from agent output JSON → validate/sanitize viz specs (max 5 per message, 50 rows per chart) → store as assistant message with `metadata: { visualizations_schema_version: 1, visualizations: [...] }` → auto-open right panel on first chart arrival. Each `pollRunStatus` call creates a self-contained poll loop with its own timeout lifecycle tracked in a `Set<Timeout>` — multiple runs across different sessions can poll concurrently without cancelling each other. Run output extraction priority: `markdown_report` > `result` > `output` > raw JSON fallback (handles both object and stringified JSON).
...
**Chat service layer:** `services/chatService.ts` wraps `/api/chat/*` endpoints. `addMessage()` accepts optional `metadata` param for persisting visualization specs. Types in `types/chat.ts` (`AgentChatSession`, `AgentChatMessage`, `ReasoningEvent`) and `types/visualization.ts` (`ChartType`, `VisualizationSpec` with `schema_version`, `value_format`, `stacked` fields).

**Visualization schema:** `VisualizationSpec` supports `bar`, `line`, `pie`, and `area` chart types via recharts. Specs are validated with `in` operator for key checks (not falsy), pie charts enforce single `y_key`. Stored in `chat_messages.metadata` JSONB (no migration needed). FormatterAgent prompt (`prompts/formatter.md`) outputs `visualizations: []` array alongside `markdown_report`. Backend enforces 100KB metadata size limit in `routers/chat.py`.
