...
export FRONTEND_ORIGIN="https://askcortex.dev"  # Firebase Hosting URL (custom domain)
...
This creates the site at `https://apexflow-console.web.app`. A custom domain `askcortex.dev` is also configured (CNAME → `apexflow-console.web.app`).
...
gcloud run services update apexflow-api --region=us-central1 \
  --update-env-vars="^||^CORS_ORIGINS=https://apexflow-console.web.app,https://apexflow-ai.web.app,https://askcortex.dev" \
  --project=apexflow-ai
```
...
2. Under **Authorized domains**, verify these are listed:
   - `localhost` (local dev)
   - `askcortex.dev` (custom domain — also used as `authDomain`)
...
VITE_FIREBASE_AUTH_DOMAIN=askcortex.dev
...
> **Important:** `AUTH_DOMAIN` must be a Firebase Hosting domain that matches where users visit. The primary domain is `askcortex.dev` (custom domain CNAME → `apexflow-console.web.app`). The `signInWithRedirect` flow redirects through the `authDomain` — using a different domain triggers third-party cookie blocking in modern browsers (Chrome 120+), breaking the sign-in flow. Firebase Hosting auto-serves `/__/auth/handler` on all hosting sites (including custom domains). The OAuth client's **Authorized redirect URIs** (in [APIs & Services → Credentials](https://console.cloud.google.com/apis/credentials)) must include both `https://askcortex.dev/__/auth/handler` and `https://apexflow-console.web.app/__/auth/handler`.
...
| Firebase Hosting | `apexflow-console` | Site in `apexflow-ai`, serves `frontend/dist`, rewrites `/api/**` to Cloud Run. Custom domain: `askcortex.dev` |
| Firebase Auth | Identity Platform | Google sign-in provider, authorized domains: localhost, `askcortex.dev`, `*.web.app` |
