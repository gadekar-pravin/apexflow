...
class BeliefUpdateEngine:
    ...
    def __init__(self, path: Path | None = None, config: BeliefUpdateEngineSchema | None = None):
    ...
    def _load(self) -> BeliefUpdateEngineSchema:
        """Load engine config from disk (if path provided and exists)."""
        if self.path and self.path.exists():
            try:
                raw = json.loads(self.path.read_text())
                return BeliefUpdateEngineSchema(**raw)
            except Exception as e:
                logger.warning("Failed to load BeliefUpdateEngine config: %s", e)
        return BeliefUpdateEngineSchema()

    def save(self) -> None:
        """Save engine config to disk (no-op when no path provided)."""
        if self.path is None:
            return
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.path.write_text(self.config.model_dump_json(indent=2))
        logger.info("Saved BeliefUpdateEngine config")
    ...
    def calculate_confidence_update(
        self, hub_name: str, current_confidence: float, is_reinforcement: bool = True, evidence_strength: str = "medium"
    ) -> float:
        """
    ...
    def calculate_decay(
        self, hub_name: str, current_confidence: float, last_updated: datetime, priority: str = "soft"
    ) -> float:
        """
    ...
    def resolve_conflict(self, belief_a: dict[str, Any], belief_b: dict[str, Any]) -> str:
        """
    ...
    def get_confidence_level(self, hub_name: str, evidence_count: int) -> str:
        """
...
_engine: BeliefUpdateEngine | None = None
